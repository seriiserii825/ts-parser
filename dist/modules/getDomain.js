...
async function addUrl() {
    const { url } = await prompt({
        ...
        validate: (val) => (isValidHTTPUrl(val) ? true : "Нужен корректный http(s) URL"),
    });
    const urls = await readUrls();
    urls.push(url.trim());
...
export { ensureUrlsFile, readUrls, writeUrls, listUrls, addUrl, editUrl, removeUrls, clearFile, FILE_NAME };
